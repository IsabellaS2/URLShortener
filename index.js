...
app.post("/", async (req, res) => {
   ...
   try {
      const checkUrlExists = await db.query("SELECT short_url FROM urls WHERE long_url = $1", [longUrl]);

      // Check if long URL exists, if it does then return the short URL
      if (checkUrlExists.rows.length > 0) {
         const shortUrl = checkUrlExists.rows[0].short_url;
         res.send(`URL already exists. Short URL: ${shortUrl}`);
      } else {
         // const addEmailToDatabase = await db.query("INSERT INTO urls (longURl) VALUES ($1)", [longURl]);

         // If URL doesn't exist, add it to the database and generate a short URL
         const shortUrl = generateShortLink();
         await db.query("INSERT INTO urls (long_url, short_url) VALUES ($1, $2)", [longUrl, shortUrl]);
         res.send(`Short URL created: ${shortUrl}`);
      }
    } catch (err) {
      console.log(err);
